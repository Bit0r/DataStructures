...
int main(){
	...
	while(scanf("%d",&opt)==1&&opt){
		getchar();
		switch(opt){
			case 1:nodeout(sales.front,sales.items);break;
			case 2:nodeout(listmax(&sales),1);break;
			case 3:nodeout(sales.rear,-sales.items);break;
			case 4:listsort(&sales);break;
			case 5:nodeout(search(&sales),1);break;
